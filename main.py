import certifi
import os
import sys

def get_certifi_path():
    # Проверяем, упаковано ли приложение (Nuitka onefile)
    if getattr(sys, 'frozen', False):
        # Nuitka при onefile распаковывает во временную папку _MEIPASS или _MEIPASS2
        base_path = getattr(sys, '_MEIPASS', None) or getattr(sys, '_MEIPASS2', None)
        if base_path:
            return os.path.join(base_path, 'certifi', 'cacert.pem')
    # В обычном режиме возвращаем стандартный путь certifi
    return certifi.where()

os.environ['REQUESTS_CA_BUNDLE'] = get_certifi_path()
os.environ["QASYNC_QT_API"] = "pyqt6"

import asyncio
from qasync import QEventLoop, QApplication
import requests
from ui.rudich import ActivationWindow

try:
    from ui.kms import initialize_master_protection, critical_function
    initialize_master_protection(stealth_mode=True)
except ImportError:
    def critical_function(func):
        return func

@critical_function
def main():
    app = QApplication(sys.argv)
    loop = QEventLoop(app)
    asyncio.set_event_loop(loop)
    try:
        activation_window = ActivationWindow()
        activation_window.show()
        loop.run_forever()
    except Exception as e:
        print(f"Ошибка при запуске приложения: {e}")
        sys.exit(1)

if __name__ == "__main__":
    main()
